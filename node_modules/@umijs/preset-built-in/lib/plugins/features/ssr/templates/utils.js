...
const handleHTML = /*#__PURE__*/function () {
  var _ref = _asyncToGenerator(function* (opts = {}) {
    ...
    const windowInitialVars = _objectSpread({}, pageInitialProps && !forceInitial ? {
      'window.g_initialProps': (0, _serializeJavascript().default)(pageInitialProps)
      ...
      if ((chunks === null || chunks === void 0 ? void 0 : chunks.length) > 0) {
        ...
        chunks.forEach(chunk => {
          Object.keys(manifest || {}).forEach(manifestChunk => {
            if (manifestChunk !== 'umi.css' && manifestChunk.includes(chunk) && /\.css$/.test(manifest[manifestChunk])) {
    ...
    if (mode === 'stream') {
      ...
      const _html$split = html.split(containerString),
      ...
      const htmlStream = (0, _mergeStream().default)(beforeRootContainerStream, containerStream, rootContainerStream, afterRootContainerStream);
      return htmlStream;
    }

    return html.replace(`<div id="${mountElementId}"></div>`, `<div id="${mountElementId}">${rootContainer}</div>`);
  });
