...
var findCurrentRouteMeta = function findCurrentRouteMeta(route, location) {
  ...
  var currentRouteMeta = (_routes$find = route.routes.find(function (currentRoute) {
    return currentRoute.path === location.pathname;
  })) === null || _routes$find === void 0 ? void 0 : _routes$find.meta;
    ...
    value: function getDerivedStateFromProps(_ref3) {
      ...
      if (navs[state.currentLocale]) {
        for (var _i = navs[state.currentLocale].length - 1; _i >= 0; _i -= 1) {
          ...
          if (nav.path && new RegExp("^".concat(nav.path.replace(/\.html$/, ''), "(/|.|$)")).test(location.pathname)) {
            navPath = nav.path;
            break;
          }
        }
      ...
      state.menus = ((_menus$state$currentL = menus[state.currentLocale]) === null || _menus$state$currentL === void 0 ? void 0 : _menus$state$currentL[navPath]) || [];
      return state;
    }
  }]);
